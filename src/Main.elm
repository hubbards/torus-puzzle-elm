...
update msg model =
  case msg of
    Shuffle   -> (model, shuffle model.numRows model.numCols)
    Cycles cs -> ({ model | grid = flip (List.foldl Puzzle.cycle) cs model.grid }, Cmd.none)
    Cycle c   -> ({ model | grid = Puzzle.cycle c model.grid }, Cmd.none)
...
shuffle numRows numCols =
  let
    num =
      30
    cycle =
      Random.andThen (\b -> if b then row else col) Random.bool
    row =
      Random.pair
        (Random.map (\b -> if b then Puzzle.East else Puzzle.West) Random.bool)
        (Random.int 0 <| numRows - 1)
    col =
      Random.pair
        (Random.map (\b -> if b then Puzzle.North else Puzzle.South) Random.bool)
...
cyclePanel num dir =
  Html.div
    ...
    <| List.map (cycleButton dir) <| List.range 0 (num - 1)
